...
dat_gen <- function(n=1000,alpha = 0.2, beta = c(1,2,2)){
  ...
  y_0 <- 1*(s=="ANT"|s=="AAT")*(1+x1+x2+z_h+u)+
    1*(s=="ATNT"|s=="NTAT")*(1+x1+x2+z_h+u)+
    1*(s=="SWI1"|s=="SWI2")*(1+x1+x2+z_h+u)+
    1*(s=="ACO")*(1+x1+x2+z_h+u)+epsilon
  ...
  y_1 <- 1*(s=="ANT"|s=="AAT")*(1+x1+x2+z_h+u)+
    1*(s=="ATNT"|s=="NTAT")*(1+x1+x2+z_h+u)+
    1*(s=="SWI1"|s=="SWI2")*(beta[1]+beta[2]*(x1)+beta[3]*(x2)+u+z_h)+
    1*(s=="ACO")*(1+2*x1+2*x2+u++z_h)+epsilon
...
dat_try <- dat_gen(2000, beta = beta_list[[1]],alpha = alpha_list[1])

res_try <- nptest(dat_try)
...
FileSave <- paste0("/home/bzhang3/Wang_Rui/nested_iv/output_test_wald_misspecification/sim_",touse,".csv")
