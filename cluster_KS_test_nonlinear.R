...
dat_gen <- function(n=1000,alpha = 0.2, beta = c(1,2,2)){
  ...
  y_1 <- 1*(s=="ANT"|s=="AAT")*(1+x1+x2+u)+
    1*(s=="ATNT"|s=="NTAT")*(1+x1+x2+u)+
    1*(s=="SWI1"|s=="SWI2")*(beta[1]+beta[2]*x1^2+beta[3]*expit(x2)+u)+
    1*(s=="ACO")*(1+2*x1^2+2*expit(x2)+u)+epsilon
...
FileSave <- paste0("/home/bzhang3/Wang_Rui/nested_iv/output_test_ks_nonlinear/sim_",touse,".csv")
